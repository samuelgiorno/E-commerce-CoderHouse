...
const CountButton = ({cant, action}) => {
    ...
    return (
        <div id='count'>
            <div className="d-flex flex-row justify-content-center align-items-center" id="cardCount"> 
                <button id="menos" className="btn mx-2 p-0" onClick={restOne} disabled={stop(1)}>-</button>

                <label id="numero" className="m-0 p-0 text-center w-100">{count}</label>
                
                <button id="mas" className="btn mx-2 p-0" onClick={addOne} disabled={stop(cant)}>+</button>
            </div>
